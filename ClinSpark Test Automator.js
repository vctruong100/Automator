...
(function () {
    ...
    function setImportInProgress(flag) {
        log("setImportInProgress: " + String(!!flag));
        try {
            localStorage.setItem(STORAGE_IMPORT_IN_PROGRESS, flag ? "1" : "0");
        } catch (e) {}
    }
    ...
    // Clear the persisted barcode result.
    ...
    // Automate locking a study library form when in run mode.
    async function processStudyLibraryFormShowLock() {
        if (isPaused()) {
            log("Paused; skipping form lock processing");
            return;
        }
        var auto = getQueryParam("autolock");
        var mode = getRunMode();
        ...
        var opened = await clickActionsDropdownIfNeeded();
        if (!opened) {
            log("Action dropdown not found on form show");
            return;
        }
        var lockLink = await waitForSelector('a[href^="/secure/crfdesign/studylibrary/locking/form/"][data-toggle="modal"]', 5000);
        if (!lockLink) {
            lockLink = document.querySelector('a[href^="/secure/crfdesign/studylibrary/locking/form/"]');
        }
        if (!lockLink) {
            log("Lock link not found");
            return;
        }
        lockLink.click();
        var modal = await waitForSelector("#ajaxModal, .modal", 5000);
        if (!modal) {
            log("Lock modal did not open");
            return;
        }
        var saveBtn = await waitForSelector("#actionButton", 5000);
        if (!saveBtn) {
            log("Lock modal Save button not found");
            return;
        }
        saveBtn.click();
        if (mode === "all") {
            setContinueEpoch();
            log("Eligibility locked in ALL; redirecting to Study Metadata");
            location.replace(STUDY_METADATA_URL);
            return;
        }
        clearRunMode();
        log("Eligibility locked; redirecting to Study Metadata");
        location.replace(STUDY_METADATA_URL);
    }
    ...
    // Log to console and to the on-page log box if present.
    ...
    // Poll until selector found or timeout; return element or null.
    ...
    // Build and inject the draggable control panel and wire up its buttons and events.
