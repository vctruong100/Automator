...
(function () {
    ...
    // Dismiss Dropdown by Clicking on the Automator directly
    function dismissRoleDropdownOnly() {
        return new Promise(function(resolve) {
            addLogMessage('dismissRoleDropdownOnly: clicking automator panel to close role dropdown', 'log');
            var automatorPanel = document.getElementById('florence-gui');
            if (automatorPanel) {
                automatorPanel.click();
                addLogMessage('dismissRoleDropdownOnly: clicked automator panel', 'log');
                var wt = setTimeout(function() {
                    resolve();
                }, 200);
                respState.timeouts.push(wt);
            } else {
                addLogMessage('dismissRoleDropdownOnly: automator panel not found, using body click', 'warn');
                document.body.click();
                var wt = setTimeout(function() {
                    resolve();
                }, 200);
    ...
    function dismissExistingResponsibilitiesMenu() {
        return new Promise(function(resolve) {
            ...
            addLogMessage('dismissExistingResponsibilitiesMenu: closing stale menu', 'log');
            document.body.click();
            var attempts = 0;
            function waitGone() {
                var still = document.querySelector(RESP_SELECTORS.responsibilitiesMenu);
                if (!still) {
                    addLogMessage('dismissExistingResponsibilitiesMenu: stale menu closed', 'log');
                    resolve();
                    return;
                }
                attempts++;
                if (attempts >= 10) {
                    addLogMessage('dismissExistingResponsibilitiesMenu: forcing removal', 'warn');
                    still.remove();
                    resolve();
                    return;
                }
        ...
        return dismissExistingResponsibilitiesMenu().then(function() {
            return new Promise(function(resolve, reject) {
                try {
                    ...
                    addLogMessage('openResponsibilitiesDropdown: clicking toggle button', 'log');
                    toggleBtn.click();
                    respWaitForElement(RESP_SELECTORS.responsibilitiesMenu, RESP_TIMEOUTS.waitResponsibilitiesMenuMs)
                        .then(function(menu) {
                        addLogMessage('openResponsibilitiesDropdown: fresh menu appeared', 'log');
                        resolve(menu);
                    })
                        .catch(function(err) {
                        addLogMessage('openResponsibilitiesDropdown: menu timeout, retrying click', 'warn');
                        toggleBtn.click();
                        respWaitForElement(RESP_SELECTORS.responsibilitiesMenu, RESP_TIMEOUTS.waitResponsibilitiesMenuMs)
                            .then(function(menu) {
                            resolve(menu);
        ...
        return new Promise(function(resolve) {
            try {
                ...
                function getMenuContainer() {
                    var menu = document.querySelector(RESP_SELECTORS.responsibilitiesMenu);
                    return menu;
                }
