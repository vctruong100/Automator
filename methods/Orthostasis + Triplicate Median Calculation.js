...
// Version: v2
// Description: Calculates the difference between a standing vital-sign value and the median of three matching supine or semi-recumbent triplicate values for orthostasis workflows.
...
function getTriplicateMedian(form, metric, isRepeat) {
...
function getOrthostasis(form, metric, isRepeat) {
    var semi = getTriplicateMedian(form, metric, isRepeat);
    if (semi === null) {
        logger("No semi/triplicate median found for metric: " + metric);
        return null;
    }
    ...
    logger("Final Semi (median of triplicates) for " + metric + ": " + semi);
